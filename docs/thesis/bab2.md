...
## 2.8 Metode SDLC Incremental

Software Development Life Cycle (SDLC) merupakan kerangka kerja sistematis yang digunakan dalam pengembangan sistem informasi dan perangkat lunak. SDLC mendefinisikan proses pengembangan atau perubahan suatu sistem perangkat lunak dengan menggunakan model dan metodologi yang terstruktur untuk menghasilkan sistem yang berkualitas. SDLC mencakup seluruh tahapan mulai dari analisis kebutuhan pengguna hingga pemeliharaan sistem setelah implementasi. Penggunaan SDLC dalam pengembangan perangkat lunak memberikan beberapa keuntungan, antara lain struktur fase pengembangan yang jelas, dokumentasi yang dibuat di setiap fase pengembangan, serta setiap fase diimplementasikan setelah fase sebelumnya selesai (Erawati, Heristian, dan Purnama, 2023).

Dalam konteks pengembangan sistem informasi akademik berbasis website, pemilihan metode SDLC yang tepat menjadi faktor kritis yang menentukan keberhasilan proyek. Metode SDLC menyediakan siklus hidup perangkat lunak yang terstruktur dan terorganisir, memungkinkan tim pengembang untuk menghasilkan sistem yang memenuhi kebutuhan pengguna dengan kualitas yang konsisten. Metodologi ini juga membantu dalam mengelola kompleksitas proyek dengan memecahnya menjadi tahapan-tahapan yang dapat dikelola secara sistematis (Manulang dan Uddin, 2026). Terdapat berbagai model dalam SDLC yang dapat digunakan, salah satunya adalah model Incremental yang dipilih dalam penelitian ini.

Model Incremental merupakan salah satu pendekatan dalam SDLC yang merupakan gabungan elemen dari model Waterfall dengan model Spiral yang keduanya sama-sama memiliki gaya yang interaktif. Model ini membagi pengembangan perangkat lunak ke dalam pengembangan komponen-komponen yang lebih kecil, kemudian komponen-komponen tersebut diintegrasikan dalam setiap pengulangan pengembangan dan diuji secara keseluruhan. Model Incremental merupakan model manajemen yang sederhana dengan risiko lebih rendah terhadap keseluruhan pengembangan sistem, serta sistem dapat segera digunakan (Kaafi, Widiastuti, dan Arsiadi, 2022).

Karakteristik utama model Incremental adalah kemampuannya untuk menghasilkan versi produk yang berfungsi secara bertahap. Setiap increment menghasilkan produk yang dapat digunakan secara mandiri, meskipun belum mencakup seluruh fitur yang direncanakan. Pendekatan ini memungkinkan pengguna untuk mulai menggunakan sistem lebih awal sambil pengembangan fitur-fitur lainnya berlanjut. Model Incremental juga memungkinkan fleksibilitas dalam perubahan kebutuhan karena setiap increment dapat disesuaikan berdasarkan umpan balik dari pengguna sebelumnya (Kaafi, Widiastuti, dan Arsiadi, 2022).

Keunggulan model Incremental terletak pada kemampuannya untuk meminimalisir risiko cacat atau bug selama proses pengembangan perangkat lunak. Hal ini terjadi karena setiap release dilakukan pengujian secara bertahap, sehingga masalah dapat diidentifikasi dan diperbaiki lebih awal sebelum terintegrasi dengan komponen lainnya. Model ini juga membantu mengurangi biaya pengembangan karena manajemen yang sederhana dan risiko untuk kegagalan proyek secara keseluruhan lebih rendah. Perangkat lunak dapat segera digunakan dengan tepat waktu dan tepat biaya (Kaafi, Widiastuti, dan Arsiadi, 2022).

Tahap dalam model Incremental secara umum meliputi analisis kebutuhan yang dilakukan untuk mengidentifikasi kebutuhan sistem secara keseluruhan dan memecahnya menjadi modul-modul yang dapat dikembangkan secara bertahap. Tahap ini melibatkan pengumpulan informasi dari stakeholder mengenai fitur-fitur yang diperlukan dan prioritas dari setiap fitur. Modul-modul dengan prioritas tinggi akan dikembangkan terlebih dahulu dalam increment pertama. Selanjutnya, desain dan implementasi dilakukan untuk setiap increment secara terpisah. Setiap increment dirancang dan diimplementasikan sebagai unit yang berdiri sendiri dan dapat berfungsi secara mandiri. Setelah desain selesai, tahap pengkodean dilakukan untuk menerjemahkan desain ke dalam bahasa pemrograman yang dipilih. Pengujian dilakukan terhadap setiap increment untuk memastikan bahwa modul yang dikembangkan telah berfungsi sesuai dengan kebutuhan yang ditetapkan. Integrasi merupakan tahap di mana increment yang telah diuji diintegrasikan dengan increment-increment sebelumnya yang telah dikembangkan. Setiap integrasi dilakukan secara bertahap dan disertai pengujian untuk memastikan bahwa increment baru tidak merusak fungsionalitas increment sebelumnya. Tahap deployment dilakukan untuk setiap increment, di mana versi sistem yang baru didistribusikan kepada pengguna untuk dapat digunakan (Kaafi, Widiastuti, dan Arsiadi, 2022).

Penerapan model Incremental pada pengembangan sistem informasi telah dibuktikan efektif melalui berbagai penelitian. Kaafi, Widiastuti, dan Arsiadi (2022) menggunakan model Incremental dalam pengembangan Sistem Informasi Pendaftaran Peserta Didik Baru (PPDB) SMA Uswatun Hasanah Jakarta. Penelitian tersebut menunjukkan bahwa model Incremental mampu memberikan sistem informasi yang dapat langsung digunakan secara bertahap tanpa harus menunggu semua modul selesai sehingga jika ada perbaikan atau penambahan fitur dapat segera dieksekusi dan tetap bisa digunakan.
